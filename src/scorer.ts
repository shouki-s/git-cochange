...
export function computeScores(commits: CommitInfo[]): ScoreMap {
  ...
  for (const authorCommits of groupByAuthor(commits).values()) {
    authorCommits.sort((a, b) => a.timestamp - b.timestamp)

    for (let i = 0; i < authorCommits.length; i++) {
      accumulateSameCommit(scoreMap, authorCommits[i])

      for (let j = i + 1; j < authorCommits.length; j++) {
        const delta = authorCommits[j].timestamp - authorCommits[i].timestamp
        if (delta >= CUTOFF_SECONDS) break
        accumulateCrossCommit(scoreMap, authorCommits[i], authorCommits[j], delta)
      }
    }
  }
